...
const GEMINI_FALLBACK_MODELS = ['gemini-flash-latest', 'gemini-3.8-flash', 'gemini-3.5-flash', 'gemini-2.5-flash'];
...
export async function gemini(prompt, { json = false, temperature = 0.4, system, model: preferred } = {}) {
  ...
  for (const model of [...new Set(models)]) {
    ...
    if (res.status === 404 || (res.status === 400 && /not found|not supported/i.test(msg))) {
      lastError = new Error(`Gemini model ${model} unavailable: ${msg}`);
      continue;
    }
