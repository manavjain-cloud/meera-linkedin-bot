...
export async function gemini(prompt, { json = false, temperature = 0.4, system, model: preferred } = {}) {
  ...
  for (const model of [...new Set(models)]) {
    ...
    // Model missing, out of quota, or temporarily overloaded: try the next one.
    if ([404, 429, 500, 503, 504].includes(res.status) || (res.status === 400 && /not found|not supported/i.test(msg))) {
