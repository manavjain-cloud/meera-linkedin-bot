...
Plain text, under 90 words, no headings, no emoji, no em dashes, no curly quotes. Start with "Score ${s.score}/10." on its own line.
...
- Never invent statistics, studies, quotes or sources. Only use numbers that appear in her note or in the news items provided.
...
export async function draftPost(note, thesis, news) {
  ...
  let post = cleanText(String(out.post || ''));
  const words = countWords(post);
  if (words < 100 || words > 200) {
    ({ out, model } = await run(
      `${prompt}\n\nYour previous draft was ${words} words. Rewrite it to land between 120 and 180 words, keeping everything else.\nPrevious draft:\n${post}`
    ));
    post = cleanText(String(out.post || ''));
  }
...
export function formatDraftMessage(post, usedNews, searchedButEmpty) {
  let msg = post;
  if (usedNews) {
    msg += [
      '',
    ...
    msg += '\n\n--------------------------------\nNo supporting news data was found for this post. No statistics were added.\n--------------------------------';
  }
...
export async function runPipeline({ note, noteId, chatId, messageId }) {
  ...
  const message = formatDraftMessage(d.post, d.usedNews, r.news.length === 0);
