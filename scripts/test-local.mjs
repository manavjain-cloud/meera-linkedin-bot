...
assert(draft.startsWith('SCORE: 8/10') && draft.includes('Why: Has a number.'), 'draft message starts with score and reason');
assert(draft.includes('Used in post: India cosmetics labelling rules tightened') && draft.includes('Link: https://example.com/a'), 'draft message lists the source');
...
assert(sent.at(-1).startsWith('SCORE: 5/10') && !sent.at(-1).includes('\u2014'), 'feedback has score header and is cleaned');
r = await call('APPROVE'); assert(r.d.ok && sent.at(-1).includes('approved'), 'APPROVE updates draft');
const { inventedNumbers } = await import('../lib/pipeline.js');
assert(JSON.stringify(inventedNumbers('In 2021 I spent 7 months; 23% failed. pH 5.6 #skin2', 'our serum is pH 5.6', null)) === '["2021","7","23"]', 'flags numbers not in the note');
